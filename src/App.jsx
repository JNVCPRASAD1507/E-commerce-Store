...
import { AppBar, Toolbar, IconButton, Badge, Typography, Box, InputBase, Container, Button, useMediaQuery, Fade, Collapse } from "@mui/material";
...
function MobileSearchBar({
  ...
  React.useEffect(() => {
    if (showSearch) {
      ...
      const t = setTimeout(() => setSearchBarVisible(false), 350);
      return () => clearTimeout(t);
    }
  }, [showSearch]);

  const showMobileSuggestions = searchBarVisible && (searchFocused || search.length > 0);

  return (
    <AppBar position="static" sx={{ background: 'linear-gradient(to right, #195046ff, #2c355bff)', color: '#fff', boxShadow: 2, zIndex: 1201 }}>
      <Toolbar sx={{ display: 'flex', justifyContent: 'space-between', px: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
          <Box
            ...
            sx={{ height: 36, width: 'auto', mr: 1, borderRadius: 1, boxShadow: '0 4px 16px 0 rgba(44,53,91,0.18)', background: 'linear-gradient(135deg, #ffe082 0%, #ff7043 100%)' }}
          />
          <Typography variant="h6" sx={{ fontWeight: 900, fontSize: '1.1rem', letterSpacing: 1, flex: 1, color: 'whitesmoke', textShadow: '0 2px 8px #19504699, 0 1px 0 #fff' }}>
            JNV Super Mart
          </Typography>

          <IconButton color="inherit" onClick={() => setShowSearch(prev => !prev)} sx={{ ml: 1 }}>
            <SearchIcon />
          </IconButton>

          {isAuthenticated && location.pathname !== "/cart" && (
            <IconButton color="inherit" onClick={() => navigate("/cart")} aria-label="View cart" sx={{ ml: 1 }}>
          ...
          {isAuthenticated && (
            <IconButton color="inherit" onClick={handleLogout} sx={{ ml: 1 }} aria-label="Logout">
              <span style={{ fontWeight: 700, fontSize: 14 }}>Logout</span>
            </IconButton>
          )}
        </Box>
      </Toolbar>
      ...
      <Box
        sx={{
          width: searchBarVisible ? 300 : 0,
          maxWidth: '100vw',
          position: 'absolute',
          top: 56,
          left: '50%',
          transform: searchBarVisible ? 'translateX(-50%) translateY(0)' : 'translateX(-50%) translateY(-30px)',
      ...
      >
        <InputBase
          ...
          onBlur={() => { setTimeout(() => { setShowSuggestions(false); setSearchFocused(false); }, 150); }}
          onKeyDown={handleSearchKeyDown}
          sx={{ width: '100%', fontSize: 15, px: 1, py: 0.5, borderRadius: 2, border: '1.5px solid #195046', background: '#fff', color: '#222' }}
          inputProps={{ 'aria-label': 'search', style: { color: '#222', background: '#fff' } }}
        />
      </Box>
          ...
          {(suggestions || []).map((item, idx) => {
            ...
            if (!s) {
              return (
                <Box
                  key={item._id}
                  id={`suggestion-${idx}`}
                  sx={{ px: 2, py: 1, cursor: 'pointer', bgcolor: highlightedIndex === idx ? '#e3f2fd' : undefined, fontSize: 15, fontWeight: highlightedIndex === idx ? 600 : 400, '&:hover': { bgcolor: '#f5f5f5' } }}
                  tabIndex={0}
                  role="option"
            ...
            return (
              ...
              >
                {parts.map((part, i) =>
                  regex.test(part) ? (
                    <span key={i} style={{ background: '#ffe082', color: '#195046', borderRadius: 2 }}>{part}</span>
                  ) : (
...
const App = () => {
  ...
  // Fix: useMediaQuery must be called at the top level, not inside render
  ...
  useEffect(() => {
    ...
    try {
      const parsedCart = storedCart && storedCart !== "undefined"
  ...
  return (
    <Box sx={{ width: '100%', minHeight: '100vh', overflowX: 'hidden', position: 'relative', pb: { xs: 8, sm: 10 } }}>
      {/* Splash Screen */}
      {showSplash && (
        <Box
          sx={{
              ...
              >
                <Toolbar sx={{ px: { xs: 1, sm: 2 }, flexWrap: 'wrap' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', flex: 1, justifyContent: { xs: 'center', sm: 'flex-end' }, minWidth: 0, mx: 2 }}>
                    ...
                    <Collapse in={showSearchBar} orientation="horizontal" sx={{
                      width: showSearchBar ? { xs: 1, sm: 320, md: 400 } : 0,
                      minWidth: 0,
                      position: 'static',
                      zIndex: 1202,
                      background: 'none',
                      ...
                      <Box sx={{ position: 'relative', width: '100%', display: 'flex', alignItems: 'center', zIndex: 1203 }}>
                        <input
                          ref={searchInputRef}
                          type="text"
                          value={search}
